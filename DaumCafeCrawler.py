...
while dt_start_1 <= dt_end_date :
    ...
    while True : 
        ...
        for a,d in zip(a_tags,span_tags) :
            ...
            cafe_title = soup.find("div", {"class" : "article_subject"}).find("span", {"class" : "b"}).text
            cafe_date = d.text
            writer = soup.find("a", {"class" : "txt_point p11"}).text
            
            # 공감 버튼이 비어있다면 0으로
            if soup.find("span", {"id" : "recommendCnt"}) is not None :
                cafe_like = soup.find("span", {"id" : "recommendCnt"}).text 
            else :
                cafe_like = 0
                    
            reply_count = soup.find("span", {"class" : "comment_cnt"}).text.strip()[3:]
            cafe_content = soup.find("div", {"id" : "bbs_contents"}).text.replace('\n','').strip()
            
            total_list.append([url,cafe_title,cafe_date,writer,cafe_like,reply_count,cafe_content])

        driver.get(p_url)
        page_num += 1
        # 페이지 게시물 수가 10개보다 작으면 while문 빠져나감
        try :
...
# DB 생성시 이용
# query = """CREATE DATABASE daum_cafe default CHARACTER SET UTF8;"""
# curs.execute(query)
